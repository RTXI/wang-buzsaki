...
wbscaled::wbscaled(void) :
  DefaultGUIModel("Wang Buzsaki - scaled", ::vars, ::num_vars)
{
...
void
wbscaled::update(DefaultGUIModel::update_flags_t flag)
{
  switch (flag)
    {
  case INIT:
...
void
wbscaled::solve(double dt, double *y)
{
  double dydt[3];
  derivs(y, dydt);
...
void
wbscaled::derivs(double *y, double *dydt)
{
